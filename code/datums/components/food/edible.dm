...
/datum/component/edible/proc/TryToEat(mob/living/eater, mob/living/feeder)
	...
	if(HAS_TRAIT(eater, TRAIT_VORACIOUS))
		if(fullness < NUTRITION_LEVEL_FULL || (eater != feeder)) // No extra delay when being forcefed
			time_to_eat *= EAT_TIME_VORACIOUS_MULT
		else
			time_to_eat *= (fullness / NUTRITION_LEVEL_FULL) * EAT_TIME_VORACIOUS_FULL_MULT // takes longer to eat the more well fed you are
