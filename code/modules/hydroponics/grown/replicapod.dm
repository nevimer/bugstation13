...
/obj/item/seeds/replicapod/harvest(mob/user) //now that one is fun -- Urist
	...
	for(var/V in quirks)
		new V(podman)
	podman.hardset_dna(null, null, null, podman.real_name, blood_type, new /datum/species/pod/sprout, features) // Discard SE's and UI's, podman cloning is inaccurate, and always make them a podman
